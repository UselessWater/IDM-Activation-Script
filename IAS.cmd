...
::
::   By UselessWater:
::              https://github.com/UselessWater/IDM-Activation-Script
::
::============================================================================
...
set "blank="
set "mas=https://github.com/UselessWater/IDM-Activation-Script/wiki/"
...
>nul findstr /v "$" "%~nx0" && (
echo:
echo Error: Script has LF line ending issue, or missing blank line at the end.
echo:
...
%nceline%
echo Unsupported OS version [%winbuild%].
echo This project supports Windows 7/8/8.1/10/11 and their server equivalents.
goto done2
)
...
%eline%
echo Script is running from the temp folder.
echo Most likely, you ran the script directly from an archive file.
echo:
...
echo:
echo PowerShell is not running in FullLanguage mode. Aborting...
echo If you have applied PowerShell restrictions, please undo those changes.
echo:
...
echo To run with admin rights, right-click this script and select “Run as administrator”.
goto done2
)
...
echo:
echo WMI is not running. Aborting...
echo:
echo Check this page for help. %mas%IAS-Help#troubleshoot
goto done2
)
...
echo:
echo [%_sid%]
echo User SID not found. Aborting...
echo:
echo Check this page for help. %mas%IAS-Help#troubleshoot
goto done2
)
...
%eline%
echo Unable to write to %CLSID2%
echo:
echo Check this page for help. %mas%IAS-Help#troubleshoot
goto done2
)
...
echo:
echo:
call :_color2 %_White% "             " %_Green% "By UselessWater"
echo:            ___________________________________________________ 
echo:
echo:               Github: 
echo:               https://github.com/UselessWater/IDM-Activation-Script
echo:            ___________________________________________________ 
echo:                                                               
echo:               [1] Activate
echo:               [2] Freeze Trial
echo:               [3] Reset Activation / Trial Status
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "Select an option using the keyboard [1,2,3,4,5,0]"
choice /C:123450 /N
set _erl=%errorlevel%

if %_erl%==6 exit /b
if %_erl%==5 start https://github.com/UselessWater/IDM-Activation-Script & goto MainMenu
...
echo:
echo Backing up registry to %SystemRoot%\Temp
...
echo %line%
echo:
call :_color %Green% "IDM activation/trial has been reset."
...
echo:
echo Deleting IDM registry info...
echo:
...
echo %line%
echo:
echo      For some users, activation may not work and may show a fake trial window.
echo:
call :_color2 %_White% “     “ %_Green% “Use the “”Freeze Trial”” option instead.”
echo %line%
echo:
choice /C:19 /N /M ">    [1] Back [9] Activate : "
...
call :_color %Gray% "Ping to internetdownloadmanager.com failed"
echo:
)
...
echo:
echo Backing up CLSID registry to %SystemRoot%\Temp
...
%eline%
echo Error: Unable to download files using IDM.
echo:
echo Help: %mas%IAS-Help#troubleshoot
goto :done
)
...
echo:
if %frz%==0 (
call :_color %Green% "IDM has been successfully activated."
echo:
call :_color %Gray% "If you see a fake trial screen, use the Freeze Trial option."
) else (
call :_color %Green% "Successfully froze IDM 30-day trial period."
echo:
call :_color %Gray% "If IDM shows a registration dialog, reinstall IDM."
)
...
echo:
echo Generating registration info...
echo:
...
echo:
echo Triggering downloads to create certain registry entries, please wait...
echo:
...
foreach ($regPath in $regPaths) {
	...
	Write-Host
	Write-Host "Scanning IDM CLSID registry keys in $regPath"
	Write-Host
    ...
    foreach ($lockedKey in $lockedKeys) {
        ...
        Write-Output "$leafValue - Found locked registry key"
    }
    ...
    foreach ($key in $filteredKeys) {
        ...
        if (($defaultValue -match "^\d+$") -and ($key.SubKeyCount -eq 0)) {
            ...
            Write-Output "$($key.PSChildName) - Found numeric default value with no subkeys"
            continue
        }
        if (($defaultValue -match "\+|=") -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - Found non-numeric default value with no subkeys"
            continue
        }
        ...
        if (($versionValue -match "^\d+$") -and ($key.SubKeyCount -eq 1)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - Found numeric in \Version, no other subkeys"
            continue
        }
        ...
        if (($key.ValueCount -eq 0) -and ($key.SubKeyCount -eq 0)) {
            $finalValues += $($key.PSChildName)
            Write-Output "$($key.PSChildName) - Found empty subkey"
            continue
        }
    }
}
...
if (($finalValues.Count -gt 20) -and ($toggle -ne $null)) {
	$lockKey = $null
	$deleteKey = 1
    Write-Host "IDM registry entries exceed 20. Deleting instead of locking..."
	Write-Host
}
